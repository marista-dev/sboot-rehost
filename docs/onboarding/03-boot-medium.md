...
> 매체를 왜 합성하는지, 어떻게 만드는지, 컨트롤러 모델이 왜 두 번 검증되는지.
...
| 주체 | 무엇을 | 비고 |
|---|---|---|
| 첫 스테이지 | 파티션 테이블 | 앞 단계가 남긴 함수 포인터로 읽는다. **컨트롤러 모델 불필요** |
| 부트로더 | boot image · 검증 자료 | 자체 드라이버로 컨트롤러 구동 |
...
| UTRD | 전송 요청 서술자. 명령·응답 버퍼와 데이터 목록을 가리킨다 |
| UPIU | 실제 명령·응답 패킷. **LUN 은 헤더 3번째 바이트** |
| PRDT | 데이터 스캐터 목록. 항목 간격이 벤더 확장으로 넓어질 수 있다 |

- LU(Logical Unit)는 물리 장치를 논리적으로 나눈 단위. 보통 부팅용과 사용자 데이터용이 분리
...
- 부트로더는 파티션을 **이름으로** 조회한다. 지어낸 이름은 영원히 찾지 못하며, 그 실패는
  한참 뒤 검증 오류나 조용한 다운로드 모드 진입으로 나타난다
...
### sparse 이미지는 디스크가 아니다

- `system.img` · `super.img` 는 AP 패키지에서 **sparse 포맷**으로 나온다
- 그대로 복사하면 부트로더가 파티션을 파싱하지 못한다
- 결함이 즉시 드러나지 않고 한참 뒤 서명 검증 실패로 나타나 원인을 찾기 어렵다
...
- 부트로더가 `console=ram` 을 고르면 커널 로그가 RAM 버퍼로 간다. **커널이 완벽히 떠도
  시리얼에 한 줄도 안 나온다**
- static-analyzer 가 `cmdline_plan.json` 에 후보를 도출 → `build_lu.py` 가 PARAM 에 기록
- **우회가 아니다.** 정상 경로(`setup_param_info` → `sbl_set_bootargs`)를 쓰고, 문자열은
  이미 펌웨어 안에 있다

### 총 크기를 바꾸면 부트로더가 디스크를 다시 만든다

**30 MiB 만 늘려도** GPT 를 재작성하고 신규 프로비저닝으로 간주해 전원을 내린다.
...
- 쓰기 가능으로 두면 회차가 이전 회차의 디스크 상태를 물려받아 실행이 멱등하지 않다
- 회차 루프는 "변하는 것은 머신 소스뿐"이라는 전제로 지문을 비교한다. 누적된 디스크
  상태는 변경 효과 판정, 정체 계산, 되돌리기를 모두 오염시킨다
...
데이터시트가 없으므로 **드라이버를 계측기로 쓴다.** 벤더 드라이버가 어떤 레지스터를
폴링하고 어떤 값을 기다리는지 관찰한 뒤, 관찰한 것만 채운다.
...
자주 만나는 정지점 (상세는 `knowledge/faults_storage.md`)
...
설계상 없다. `.ko` 부재만으로 불가 판정을 내리면 도달 가능한 실행을 거부하는 것이다.
`.ko` 도 없고 커널 이미지에도 없을 때만 `BLOCKED_KO`.
...
- `super.img` 가 없는 펌웨어는 구조상 마지막 줄을 낼 수 없으므로 목표에 넣지 않는다
