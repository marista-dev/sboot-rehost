...
> 루프가 무엇을 반복하는지, 정지를 어떻게 판정하는지, 허위 통과를 막는 장치가 무엇인지.

---

## 1. 한 문장

> **실행해서 최초 예외를 보고 → 정지점에 이름을 붙이고 → 한 곳만 고치고 → 다시 실행한다.**
...
원인은 `fingerprint.json` 의 `origin` 에 있다. 마지막 FAR/ELR 은 보존하되 진단 입력으로
쓰지 않는다. 예외 수는 자릿수로 비교한다 (2.86M 과 2.88M 은 같은 관측).

### 부팅 깊이

- 목표 단계가 한 칸이면 마일스톤이 비어 있어도 전진할 수 있다
- **콘솔 고유 줄 수**가 그 깊이를 센다. 재시도 루프가 같은 줄로 수백 KB 를 내므로
  바이트 수는 지표가 못 된다
- `timeout_bound=true` 는 더 오래 돌리니 콘솔이 더 나왔다는 뜻이다. 한계가 펌웨어가
  아니라 실행 시간이므로 fixer 를 배정하지 않는다

### 실행되지 않은 회차는 회차가 아니다

- QEMU 가 시작조차 못 하면 지문이 전부 0 으로 안정된다
- 이는 정체로 읽혀 "구조상 도달 불가" 판정이 된다. 실행된 적 없는 펌웨어에 대해서
- 실행 스크립트가 종료코드와 트레이스·콘솔 0바이트를 검사해 `BLOCKED_ENV` 로 정지시킨다
- 다만 **콘솔이 나온 뒤의 비정상 종료는 머신 결함이다.** QEMU 가 assert 로 죽어도
  게스트가 이미 출력을 냈다면 실행은 된 것이므로 `qemu_abort` 로 fixer 에 배정한다
...
- 전제가 틀리면 fixer 는 증상만 처치한다. 벡터 베이스를 망가뜨리는 명령을 NOP 으로
  바꾸는 것은, 잘못된 EL 로 진입시킨 것이 원인일 때 다음 증상을 남길 뿐이다
- **한 정지점으로 수십 회차를 소모하는 상황이 이렇게 생긴다**
- 효과 없는 변경이 임계를 넘으면 계층 재검토가 요청되고, supervisor 가 라우팅 전에
  머신 소스와 `stage_map.json` 을 읽어 판정한다
...
- "fixer 소진" 도 **실제로 질의한 답변**이어야 한다. 아무도 부르지 않은 회차를 전원
  포기로 기록하면 소진 조건이 거짓으로 성립한다
...
- **규칙 1**: "12회 읽은 뒤 값을 바꾼다" 같은 응답은 드라이버를 눈에 띄게 전진시키지만,
  기기에서라면 가지 않았을 분기로 보내며 다른 펌웨어에서 재현되지 않는다.
...
**부작용 항목은 이후 실행이 정체될 때 가장 먼저 참조된다.** 형식적으로 채우면 정작
필요한 시점에 쓸모가 없다.
...
목적은 하나다. **머신이나 에이전트가 만들어 낸 콘솔이 진짜 부팅으로 읽히지 않게 하는 것.**
...
- **`VERIFIED` 는 출처가 확인됐다는 뜻이지 부팅 완주가 아니다.** 도달은 마일스톤으로 본다
- 항목 2 에서 `%d`·`%s` 로 조립된 런타임 값은 대조하지 않는다. 콘솔의 숫자가 펌웨어에
  없는 것은 정상이며, 그것을 미발견으로 세면 진짜 콘솔이 실패로 뜬다
- 대조 대상은 같은 UART 를 쓰는 성분 전부다 (부트로더 · ldfw · tzsw · ACPM · 커널)

### 측정만 하고 판정을 막지 않는 것

체인 트레이스 · 검증 양방향 · 스토리지 이중 구동 · 우회 기록.
전부를 통과 조건으로 두면 실제 진전이 실패 판정 하나에 묻힌다.

뒤의 둘은 실패해도 뜻을 정확히 적어야 한다.

- **항상 통과하는 검증기는 항상 통과하는 스텁과 구별되지 않는다.** 훼손 시험을 안 했으면
  "통과했다"가 아니라 "검증 여부가 확인되지 않았다"고 적는다
- **한 드라이버에 맞춘 스토리지 모델은 그 드라이버가 기대하는 값의 모델이다.**
  부트로더 쪽만 동작했으면 그렇게 적는다

---

## 7. 기록

무엇을 왜 시도했고 어떻게 풀렸는지가 남아야 재개와 재현이 가능하다.
...
- **사용자 입력은 요약하지 않고 원문 그대로** 남긴다. 실행이 멈췄을 때 어떤 지시로 그
  방향을 택했는지가 가장 먼저 사라지는 정보다
- **fixer 를 지정한 회차는 변경 사유가 필수다.** 없으면 나중에 무엇을 왜 바꿨는지
  되짚을 수 없다
