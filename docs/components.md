...
- 아키텍처 시그니처가 없으면 종료코드 3. **"스테이지 없음"이 아니라 "도구 없음"** 이며
  파이프라인은 `BLOCKED_ARCH` 로 정지한다
...
| `trace_filter.py` | QEMU 트레이스를 스트리밍으로 걸러 필요한 부분만 남긴다 (회차당 10 GB → 수 MB) |
...
**게이트 3항을 모두 통과해야 `VERIFIED`.** 2단계에서 verifier 가 재검증하며, 낮추는 것은 자유롭고 올리는 것은
바이트 수준 증거가 있을 때만이다.
