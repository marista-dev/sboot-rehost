...
- **Q2 등급**: 판별한 표면·계열에 맞춰 **설명을 바꿔서** 제시한다.

  **트랙 1** — 등급은 표면 위에서 얼마나 깊이 동작하는가다:

  | 등급 | `shell` 표면일 때 | `fastboot` 표면일 때 |
  |---|---|---|
  | **A** (권장) | 프롬프트 도달 + `help` 출력 | `getvar:` 수신·에코·dispatch |
  | **B** | `reset`·`printenv` 등 핸들러 동작 | `flash`·`reboot` 등 핸들러 동작 |
  | **C** | autoboot 진행 | 부트모드 결정 → 커널 로드 |

  표면이 `미확정` 이면 "분석이 표면을 확정한 뒤 그 표면 기준으로 적용됩니다" 라고
  덧붙인다. B/C 는 `milestone_tokens.txt` 가 있어야 관측되므로, 그 사실도 안내한다.

  **트랙 2** — 등급은 스토리지 컨트롤러의 완성도다:
