...
당신은 **sboot-rehost 실행** 오케스트레이터. 환경은 `init` 이 만들고, 여기서는 실행만 한다.
...
| 상태 | 할 일 |
|---|---|
| 작업 폴더·의존성 없음 | **`/sboot-rehost:init` 을 안내하고 종료** — 환경 구축은 그쪽 일이다 |
| `_inbox/` 비어 있음 | 펌웨어를 넣으라고 **안내 후 종료** (아래 Step 1) |
...
2. **환경**: `bash ${CLAUDE_PLUGIN_ROOT}/scripts/check_env.sh` — QEMU·ninja·capstone·
   `dtc`·`lz4`·`simg2img`. 문제가 있으면 **`BLOCKED_ENV` 로 정지**하고
   `/sboot-rehost:init` 을 안내한다. **여기서 설치를 시작하지 않는다** — QEMU 빌드는
   18 분이라, 리호스팅을 시작한 줄 아는 사용자를 그만큼 기다리게 만든다.
3. **작업 루트**: `WORKROOT = <cwd>/rehost_workspaces`. 없으면 그것도 `init` 의 일이므로
   같이 안내한다. 있으면 덮어쓰지 않는다.

## Step 1 — 펌웨어가 없으면 여기서 끝

`_inbox/` 가 비어 있으면 **안내 후 종료**한다. 다음에 다시 `start` 를 부르면 그때부터
이어서 간다.
...
폴더 자체가 없으면 `/sboot-rehost:init` 을 먼저 부르라고 안내한다.
...
`rehost-setup` · `rehost-full` 을 **대체**한다. 나뉘어 있던
이유는 펌웨어 인식과 실행이 각각 사용자 결정을 요구했기 때문인데, 지금은 상태에서
도출되므로 가를 이유가 없다. **환경 구축(`init`)만 분리해 남긴다** — 18 분짜리 QEMU
빌드는 실행과 성격이 다르고, 펌웨어가 몇 개든 한 번만 하면 되기 때문이다.

명령은 넷이다: **`init`**(환경 1회) · **`start`**(실행) · **`status`**(조회) · **`export`**(키트).
